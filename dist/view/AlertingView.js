...
export class AlertingView extends BaseView {
    ...
    drawAlertPoints(points, animTick, selectedPoint) {
        ...
        for (const pt of points) {
            ...
            this.drawCenteredText(String(pt.colorIndex + 1), pt.x, pt.y, `bold ${isSelected ? 26 : 18}px sans-serif`, '#000000');
        }
    }
    /** 알림 텍스트 */
    drawAlertText(animTick) {
        const alpha = 0.7 + (animTick / 20) * 0.3;
        this.drawGlowText('🎯 뽑는 중...', this.width / 2, this.height / 2, `bold ${Math.round(this.width * 0.055)}px sans-serif`, `rgba(255,255,180,${alpha.toFixed(2)})`, '#ffff44');
    }
}
