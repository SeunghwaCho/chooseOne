...
export class SelectedView extends BaseView {
    ...
    drawCenterMode(pt, animTick) {
        ...
        this.drawCenteredText('🏆', pt.x, pt.y, '28px serif', '#ffffff');
    }
    /** HOLE 모드: 방사형 라인 */
    drawHoleMode(pt, animTick) {
        ...
        for (let deg = 0; deg < 360; deg += 15) {
            ...
            this.ctx.beginPath();
            this.ctx.moveTo(pt.x + Math.cos(rad) * innerR, pt.y + Math.sin(rad) * innerR);
            this.ctx.lineTo(pt.x + Math.cos(rad) * (innerR + lineLength), pt.y + Math.sin(rad) * (innerR + lineLength));
            this.ctx.stroke();
        }
        this.ctx.restore();
        // 중심 원
        this.fillCircle(pt.x, pt.y, 30, pt.color, 1.0);
        this.drawCenteredText('🎉', pt.x, pt.y, '22px serif', '#ffffff');
    }
    /** 당첨자 발표 텍스트 */
    drawWinnerAnnouncement(pt, animTick) {
        ...
        // 당첨 번호 표시
        this.drawGlowText(`✨ ${pt.colorIndex + 1}번 당첨! ✨`, this.width / 2, this.height * 0.12, `bold ${fontSize}px sans-serif`, '#ffffff', pt.color);
        // 색상 이름 힌트
        this.drawCenteredText(`(${this.getColorName(pt.colorIndex)})`, this.width / 2, this.height * 0.12 + fontSize + 10, `${Math.round(this.width * 0.025)}px sans-serif`, 'rgba(200,200,255,0.8)');
    }
